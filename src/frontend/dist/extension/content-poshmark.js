// Copie Past-e Smart Post — Poshmark Content Script v1.0.0
// Runs on: https://poshmark.com/create-listing
...
(() => {
  const LOG = "[Copie Past-e Poshmark]";
  ...
  function showBanner(failedFields) {
    ...
    Object.assign(banner.style, {
      ...
      borderBottom: "3px solid #cf1b45", color: "#ffffff",
      padding: "12px 20px", fontFamily: "system-ui, sans-serif",
      fontSize: "14px", display: "flex",
      justifyContent: "space-between", alignItems: "center",
      boxShadow: "0 2px 20px rgba(207, 27, 69, 0.25)",
    });
    const msg = document.createElement("span");
    if (!failedFields || failedFields.length === 0) {
      msg.innerHTML = `<span style="color:#cf1b45;font-weight:bold">⚡ Copie Past-e</span> — All fields filled. Review and click <strong>Next</strong>.`;
    } else {
  ...
  async function fillCategory(rawCategory) {
    ...
    const opts = document.querySelectorAll('[role="option"], [role="menuitem"], li[data-testid*="category"], li');
  ...
  async function autofill(data) {
    const fields = (data.platformFields?.poshmark) || data;
    const failed = [];
    ...
    try {
      const priceEl = await pollForElementPromise([
        'input[data-vv-name="original_price"]',
    ...
    try {
      if (fields.brand) {
        ...
        if (brandEl) {
          ...
          // Brand is optional on Poshmark — no push to failed
        }
      }
    } catch (err) {
  ...
  chrome.storage.local.get("pendingPost", (result) => {
    ...
    if (listing.platform && listing.platform !== CONFIG.PLATFORM) {
      log(`Post is for: ${listing.platform} — not Poshmark, exiting.`);
      return;
    }
  ...
  log("Poshmark content script loaded");
})();
