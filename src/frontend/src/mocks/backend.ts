...
export const mockBackend: backendInterface = {
  ...
  deleteBackup: async (_backupId: string) => ({ __kind__: "ok" as const, ok: null as null }),
  ...
  transformStripeBackupPaymentIntentResponse: async (raw) => raw.response,
  transformStripeGasPaymentIntentResponse: async (raw) => raw.response,
  ...
  adminForceResaveStripeConfig: async () => ({ __kind__: "ok" as const, ok: "Re-saved" }),
  adminListExtensionVersions: async () => [] as import("../backend.d").ExtensionVersion[],
  adminLockBackupPermanent: async (_backupId: string) => ({ __kind__: "ok" as const, ok: null as null }),
  adminSetExtensionVersion: async (
    _version: string,
    _buildNumber: bigint,
    _releaseNotes: string,
    _downloadUrl: string,
    _isForceUpdate: boolean,
  ) => ({ __kind__: "ok" as const, ok: "Version set" }),
  adminTestAndVerifyStripeConfig: async () => ({
    testPassed: false,
    modeCorrect: false,
    pubKeyPresent: false,
    secretKeyPresent: false,
    message: "Mock: not configured",
    configValid: false,
  }),
  checkExtensionUpdateStatus: async (_clientVersion: string): Promise<import("../backend.d").ExtensionUpdateCheck> => ({
    needsUpdate: false,
    downloadUrl: "",
    releaseNotes: "",
    currentVersion: _clientVersion,
    latestVersion: _clientVersion,
    buildNumber: BigInt(0),
    isForceUpdate: false,
  }),
  adminSetExtensionConfig: async () => ({ __kind__: "ok" as const, ok: "Config updated" }),
  adminSetPlatformCapabilities: async () => ({ __kind__: "ok" as const, ok: "Capabilities updated" }),
  getExtensionConfig: async (): Promise<import("../backend.d").ExtensionConfigResult> => ({
    latestVersion: "1.3.1",
    buildNumber: BigInt(4),
  ...
  debugConfigHealthReport: async () => ({
    status: "ok",
    allCriticalKeysPresent: false,
    siteBaseUrlSet: "",
    stripePublishableKeyLength: BigInt(0),
    geminiKeyLength: BigInt(0),
    stripSecretKeyLength: BigInt(0),
    stripeModeSet: "",
  }),
  getLatestExtensionVersion: async (): Promise<import("../backend.d").ExtensionUpdateCheck | null> => null,
  previewVersionRestoreSnapshot: async (_backupId: string) => null,
  validateBackupIntegrity: async (_backupId: string) => ({
    valid: false,
    error: "Mock: not implemented",
    listingCount: BigInt(0),
    userCount: BigInt(0),
  }),
  validateCriticalConfig: async () => ({
    status: "ok",
    keysConfigured: false,
